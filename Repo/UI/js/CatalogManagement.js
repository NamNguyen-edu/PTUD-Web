document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function (e) {
        // Tìm nút xóa gần nhất với vị trí click
        const delBtn = e.target.closest('.btn-delete');
        if (delBtn) {
            const row = delBtn.closest('tr');
            // Lấy cái tên để hiện lên thông báo cho thân thiện
            const name = row.querySelector('.item-name')?.innerText || "mục này";

            // Hiện pop-up xác nhận của trình duyệt
            if (confirm(`Ní chắc chắn muốn xóa "${name}" không?`)) {
                row.style.transition = "0.3s";
                row.style.opacity = "0"; // Hiệu ứng mờ dần trước khi biến mất
